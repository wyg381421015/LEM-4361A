...
#define	LogBufferLenMax 4096

__align(4) char LogBuffer[LogBufferLenMax];
__align(4)  rt_uint32_t LogBufferLen=0;
#include "stdio.h"    //??vsnprintf??
#include "string.h"   //??strcat??

void rt_lprintf(const char *fmt, ...)
{
    va_list args;
    rt_size_t length;
    ...
    va_end(args);
				...
				/**********************************************************************************************/
/**********************************************************************************************/
	__align(4) static char timestamp_Printf[10];
	sprintf((char*)timestamp_Printf,"[%02X%02X-%02X%02X%02X]",System_Time_STR.Month,System_Time_STR.Day,\
														         System_Time_STR.Hour,System_Time_STR.Minute,System_Time_STR.Second);
	strcat((char*)LogBuffer,(const char*)timestamp_Printf);
	
	strcat((char*)LogBuffer,(const char*)rt_log_buf);
	//check LogBuffer whether memout
	LogBufferLen = strlen((const char*)LogBuffer);
	if(LogBufferLen > (LogBufferLenMax-256))
	{
		strcpy((char*)LogBuffer,"");//??LogBuffer??,strlen(LogBuffer) = 0;
//		rt_kprintf("(%s)LogBuffer??\n",__FUNCTION__);
	}
}
